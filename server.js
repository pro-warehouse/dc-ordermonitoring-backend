...
app.post('/api/run', async (req, res) => {
    ...
    try {
        ...
        // 🟢🟢🟢 ฟังก์ชัน Master Data 6 ตัว ที่ย้ายมาจาก Google Apps Script (เดิมเรียก GAS_URL แล้ว 404) 🟢🟢🟢
            ...
            // 🟢 [FIX] target_date เป็นชนิด DATE -> ใช้ alreadyDate=true (กันไม่ให้ครอบ PARSE_DATE ตอนเลือก 'All')
            //   และขยายช่วง default เป็น 90 วัน เพื่อให้ฝั่งหน้าเว็บมีประวัติ Cap มากพอสำหรับ "ยึดค่าล่าสุด" (carry-forward)
            const dateFilter = buildDateFilter(args[0], args[1], `target_date`, 90, true);
            ...
            // 🟢 [FIX] แปลง target_date (BigQueryDate object) เป็นสตริง 'YYYY-MM-DD' ก่อนส่งกลับ
            //   เดิมส่ง object ดิบ ทำให้ฝั่งหน้าเว็บเอาไปทำเป็น key แล้วกลายเป็น "[object Object]"
            //   -> การยึด Cap ตามวันที่ (และ carry-forward) เพี้ยนทั้งหมด
            const cleaned = rows.map(r => ({
                target_date: (r.target_date && r.target_date.value) ? r.target_date.value : r.target_date,
                owner: r.owner,
                capacity: r.capacity
            }));
            result = { success: true, data: cleaned };
        }
        ...
        // =====================================================================
        // 🟢 [ใหม่] UPH TARGET & COST BASE — เก็บแบบเดียวกับ Cap (ราย target_date + ยึดค่าล่าสุด/carry-forward ฝั่งหน้าเว็บ)
        //   ตาราง: logistics_db.daily_uph_cost (สร้างอัตโนมัติถ้ายังไม่มี ตอน Save ครั้งแรก)
        // =====================================================================
        else if (fn === 'apiGetUphCost') {
            try {
                const dateFilter = buildDateFilter(args[0], args[1], `target_date`, 180, true);
                const sql = `SELECT target_date, trg_all, trg_full, trg_half, trg_ea, cost_salary, cost_days
                             FROM \`pro-analytics-db.logistics_db.daily_uph_cost\` WHERE 1=1 ${dateFilter} ORDER BY target_date`;
                const [rows] = await bigquery.query({ query: sql });
                const cleaned = rows.map(r => ({
                    target_date: (r.target_date && r.target_date.value) ? r.target_date.value : r.target_date,
                    trg_all: r.trg_all, trg_full: r.trg_full, trg_half: r.trg_half, trg_ea: r.trg_ea,
                    cost_salary: r.cost_salary, cost_days: r.cost_days
                }));
                result = { success: true, data: cleaned };
            } catch (e) {
                // ถ้าตารางยังไม่ถูกสร้าง (ยังไม่เคย Save) ให้คืน list ว่าง เพื่อให้หน้าเว็บใช้ค่า default แทน (ไม่ throw 500)
                console.warn("⚠️ [apiGetUphCost] ยังไม่มีข้อมูล/ตาราง:", e.message);
                result = { success: true, data: [] };
            }
        }
        else if (fn === 'apiSaveUphCostBulk') {
            const dataList = args[0];
            if (!dataList || dataList.length === 0) {
                result = { success: true };
            } else {
                const num = (v) => (isNaN(parseFloat(v)) ? 0 : parseFloat(v));
                // 1. สร้างตารางถ้ายังไม่มี (ผู้ใช้ไม่ต้องไปสร้างเองใน BigQuery)
                await bigquery.query({
                    query: `CREATE TABLE IF NOT EXISTS \`pro-analytics-db.logistics_db.daily_uph_cost\` (
                                target_date DATE, trg_all FLOAT64, trg_full FLOAT64, trg_half FLOAT64,
                                trg_ea FLOAT64, cost_salary FLOAT64, cost_days FLOAT64 )`
                });
                // 2. ลบข้อมูลเก่าของ "วันที่" ที่จะบันทึก แล้วเพิ่มใหม่ (เหมือน Cap)
                const dates = [...new Set(dataList.map(d => `'${d.target_date}'`))].join(',');
                await bigquery.query({
                    query: `DELETE FROM \`pro-analytics-db.logistics_db.daily_uph_cost\` WHERE target_date IN (${dates})`
                });
                const valuesStr = dataList.map(d =>
                    `('${d.target_date}', ${num(d.trg_all)}, ${num(d.trg_full)}, ${num(d.trg_half)}, ${num(d.trg_ea)}, ${num(d.cost_salary)}, ${num(d.cost_days)})`
                ).join(',');
                await bigquery.query({
                    query: `INSERT INTO \`pro-analytics-db.logistics_db.daily_uph_cost\`
                            (target_date, trg_all, trg_full, trg_half, trg_ea, cost_salary, cost_days) VALUES ${valuesStr}`
                });
                result = { success: true };
            }
        }
...
async function apiGetWaveMonitoring(startDate, endDate) {
    ...
    // 🟢 [FIX #A2] เดิมกรอง/จัดกลุ่มด้วย DATE(Created_At) (วันที่ "สร้างเรคคอร์ด")
    //   แต่หน้า Dashboard ส่ง "วันที่ตามแผน (Planned_Pick_Date)" มาให้ ทำให้ query ไม่เจอแถว
    //   -> total_orders = 0 -> กล่อง LATE ORDERS / CURRENT STATUS เลยตกไปใช้ fallback แล้วขึ้น 0
    //   แก้เป็นกรอง/จัดกลุ่มด้วย Planned_Pick_Date ให้ตรงกับที่หน้าเว็บถาม (และตรงกับ GAS)
    const dateFilter = buildDateFilter(startDate, endDate, `Planned_Pick_Date`, 7, true);
    ...
    const sql = `
        ...
        SELECT
            Planned_Pick_Date AS work_date,
