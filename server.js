...
// 🛡️ ช่วยตีความ startDate ที่หน้าเว็บอาจส่งมาเป็น 'All' / 'all' / ค่าว่าง ให้เป็น "ไม่กรองวันที่ (ย้อนหลัง 30 วัน)"
// เดิมมีการดักไว้เฉพาะ apiGetMissedPickReport ทำให้ apiGetDashboardSummary / apiGetMismatchReport / apiGetWaveMonitoring
// พังเป็น 500 เวลากดปุ่ม "วันที่ All" เพราะเอาคำว่า 'All' ไปเทียบกับ DATE ตรงๆ ใน SQL
function buildDateFilter(startDate, endDate, dateExpr, days = 30) {
    if (!startDate || startDate === 'All' || startDate === 'all' || !endDate || endDate === 'All' || endDate === 'all') {
        return ` AND ${dateExpr} >= DATE_SUB(CURRENT_DATE(), INTERVAL ${days} DAY)`;
    }
...
async function apiGetMissedPickReport(startDate, endDate) {
    ...
    const dateFilter = buildDateFilter(startDate, endDate, `PARSE_DATE('%Y-%m-%d', LEFT(CAST(PickDate AS STRING), 10))`);
...
async function apiGetDashboardSummary(startDate, endDate) {
    const datasetId = 'logistics_db';
    // 🟢 แก้บั๊ก: เดิมไม่ดัก 'All' ทำให้ SQL ได้ BETWEEN 'All' AND 'All' (คำตอบว่างเปล่า ไม่ error แต่ผิด)
...
async function apiGetWaveMonitoring(startDate, endDate) {
    ...
    const dateFilter = buildDateFilter(startDate, endDate, `DATE(Created_At)`, 7);
