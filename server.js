...
async function apiGetMissedPickReport(startDate, endDate) {
    ...
    // 🟢 แก้บัก: เดิม HAVING เรียก SUM(...ReqQty...) ซ้ำ แต่ SELECT ตั้ง alias "ReqQty" ทับชื่อคอลัมน์เดิม ทำให้ BigQuery ตีความ HAVING เป็น SUM(SUM(...))
    // แล้ว throw "Aggregations of aggregations are not allowed" → แก้โดยแยกส่วน aggregate ออกเป็น CTE ก่อน แล้วค่อย filter ด้วย WHERE ที่ query ชั้นนอก
    const sql = `
        WITH agg AS (
            SELECT
